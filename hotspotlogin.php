...
if ($_GET['login'] == login) {
  ...
  $response = md5("\0" . $hasil['password'] . $newchal);
  $newpwd = pack("a32", $hasil['password']);
...
if( hash('sha256', $hasil['password'].$_GET['md5chal']) == $_GET['Password']){
...
if ($_GET['res'] == success) {
  ...
  $headline = 'Logged in to HotSpotS';
  $bodytext = 'Welcome';
...
if ($_GET['res'] == notyet) {
  ...
  $titel = '';
  $headline = '';
  $bodytext = '<a style="color:#fff">Please Login...</a>';
  print_header();
  print_body();
  print_login_form();
  print_footer();
...
div.fixed {
    ...
    box-shadow: 0px 0px 10px #46A3FF;
    color: #FFF;
    font-family: arial;
...
$md5chal=unique_id(4);
  print '<FORM name="form1">
	  ...
	  <INPUT TYPE="HIDDEN" NAME="md5chal" VALUE="'.$md5chal.'">

          <center>
<div class="myclass"><br>
          <table border="0" cellpadding="5" cellspacing="0" style="width: 217px;color:#fff">
          <tbody>
            <tr>
            ...
            <tr>
              <td align="center" colspan="2" height="23"><input type="button" class="mybutton" name="login" value="login"> </td>
          </tr>
        </tbody>
        </table>
</div>
...
$(document).ready(function(){
	$(".mybutton").click(function(){
		var loginpatth="<?php echo $loginpath."?";  ?>";
		var mycript= SHA256( $("input[name=Password]").val() +  $("input[name=md5chal]").val() );
		$("input[name=Password]").val( mycript );
		//alert(  "http://"+$('input[name=uamip]').val()+loginpatth+$('form').serialize()+"&login=login" );
		window.location="http://"+$('input[name=uamip]').val()+loginpatth+$('form').serialize()+"&login=login";
	});
});
</script>
<?php
